...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        x = x + Variable(
            self.pe[:, self.start_indx : self.start_indx + x.size(1)],
            requires_grad=False,
        )
...
class Batch_Loss(nn.Module):
    ...
    def forward(self, w, y):  # w:[128,1,12]   y:[128,11,4]
        ...
        wt1 = w[1:]  # [128,1,12]
        pure_pc = (
            1 - torch.sum(torch.abs(wt - wt1), -1) * self.commission_ratio
        )  # [128,1]
...
class Test_Loss(nn.Module):
    ...
    def forward(self, w, y):  # w:[128,10,1,12] y(128,10,11,4)
        ...
        pure_pc = pure_pc.to(device)
        pure_pc = torch.cat(
            [torch.ones([pure_pc.size()[0], 1, 1]).to(device), pure_pc], 1
        )  # [128,1,1] cat  [128,9,1] ->[128,10,1]
        pure_pc = pure_pc.view(
            pure_pc.size()[0], pure_pc.size()[1], 1, pure_pc.size()[2]
        )  # [128,10,1] ->[128,10,1,1]
        ...
        if not self.size_average:
            ...
            SR = sr_reward.mean() / (sr_reward.std() + 1e-8)
    ...
    for i in range(total_step):
        ...
        with torch.no_grad():
            if i % output_step == 0 and evaluate:
                ...
                elapsed = time.time() - start
                print(
                    "Test: %d Loss: %f| Portfolio_Value: %f | SR: %f | CR: %f | TO: %f |testset per Sec: %f"
                    % (
                        i,
                        tst_loss.item(),
                        tst_portfolio_value.item(),
                        SR.item(),
                        CR.item(),
                        TO.item(),
                        1 / elapsed,
