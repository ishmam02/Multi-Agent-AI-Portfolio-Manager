"""
Fundamentals Analyst backtest — regime-aware, parallelised directional accuracy test.

Samples historical dates for a basket of large-cap stocks across market regimes,
runs the fundamentals analyst in parallel, and scores predicted mu.long_term
sign against realised 252-day forward returns.

Usage:
    python -m src.backtest.fundamentals_analyst_backtest --workers 4 --depth shallow
"""
...
def _get_node(depth: str):
    """Return a thread-local fundamentals-analyst node."""
    key = f"fundamentals_node_{depth}"
    if not hasattr(_thread_local, key):
        ...
        setattr(_thread_local, key, create_fundamentals_analyst(llm, ca, depth, active_horizons=("long_term",)))
        ...
        for label, r_start, r_end, n in regimes:
            ...
            for i in chosen:
                all_test_dates.append(
                    (ticker, dates_list[i], float(prices[i]), float(prices[i + forward_days]), label)
                )
...
def _run_once(args: tuple, depth: str) -> dict:
    ...
    try:
        ...
        rep = ResearchReport.model_validate_json(r.get("fundamentals_report", "{}"))
        ...
        return {"ticker": ticker, "date": d, "regime": reg, "error": str(exc), "ok": False}
...
def main():
    parser = argparse.ArgumentParser(description="Fundamentals analyst backtest")
    parser.add_argument("--tickers", nargs="+", default=DEFAULT_TICKERS, help="Stock tickers")
    parser.add_argument("--forward-days", type=int, default=252, help="Forward return horizon")
    ...
    print(
        f"FUNDAMENTALS ANALYST BACKTEST — {len(args.tickers)} tickers  "
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {
            pool.submit(_run_once, td, args.depth): td for td in test_dates
        }
        for fut in as_completed(futures):
            ...
            if res["ok"]:
                ...
                errors += 1
                print(f"  [{res['regime']}] {res['ticker']} {res['date']}  ERROR: {res['error']}")
